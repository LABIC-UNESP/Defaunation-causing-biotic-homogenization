---
title: "Itamambuca site analysis (ITA)"
...
This R Markdown document reproduces the analyses for **ITA** presented in the paper:

> **Mammal defaunation leads to biotic homogenization of plant communities in tropical rainforests**  
> Luiz Guilherme dos Santos Ribas, Nacho Villarb, Valesca Zipparro, Sérgio Nazareth, Yuri Souza, Carlos Rodrigo Brocardo, Gabriela Schmaedecke, Luana Hortenci, Rafael Souza Cruz Alves, Mauro Galetti.

## Before you run

1. Install required R packages (see the first code chunk).
2. Place the corresponding Excel file in the repository folder `data/`.
3. The script will save outputs to your **Desktop** (Windows) in a folder named `Defaunation_biotic_homogenization/ITA`.
...
**Expected input file:** `data/Itamambuca_T0_T156.xlsx`
...
output_dir <- file.path(desktop_path, "Defaunation_biotic_homogenization", "ITA")
...
excel_file <- "Itamambuca_T0_T156.xlsx"
# Data folder (relative to repository root)
data_dir <- "data"
excel_file_path <- file.path(data_dir, excel_file)

# Define the sheet names
sheet_names <- c("Itamambuca T0", "Itamambuca T6", "Itamambuca T12", "Itamambuca T18", "Itamambuca T24", "Itamambuca T30", "Itamambuca T36", "Itamambuca T42", "Itamambuca T48", "Itamambuca T54", "Itamambuca T60", "Itamambuca T66", "Itamambuca T72", "Itamambuca T78", "Itamambuca T84", "Itamambuca T96", "Itamambuca T108", "Itamambuca T156")
...
empty_df <- data.frame(matrix(NA, nrow = 18, ncol = 18))
...
for (i in seq_along(sheet_list)) {
  ...
  # Extract treatment data from each data frame (assuming it's in the third column)
...
#  Time = c("24", "30", "36", "42", "48", "54", "60", "66", "72", "78", "84", "96", "108", "156"),  # Actual significant months
#  Treatment = "Defaunation",
#  y_position = c(23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23),  # Position for the annotations, adjust as needed
#  label = "*"
#)
...
ggplot(richness_summary_data, aes(x = Time, y = mean_distance, color = Treatment, group = Treatment, fill = Treatment)) +
  ...
  scale_y_continuous(labels = scales::number_format(accuracy = 1), limits = c(5, NA), breaks = seq(5, 24, by = 3)) +
  theme(
...
#  Time = c("24", "30", "36", "42", "48", "54", "60", "66", "72", "78", "84", "96", "108", "156"),  # Actual significant months
#  Treatment = "Defaunation", 
#  y_position = c(3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3),  # Position for the annotations, adjust as needed
#  label = "*"
#)
...
ggplot(shannon_summary_data, aes(x = Time, y = mean_distance, color = Treatment, group = Treatment, fill = Treatment)) +
  ...
  scale_color_manual(values = custom_colors, labels = c("Control", "Defaunation")) +
  scale_fill_manual(values = custom_colors, labels = c("Control", "Defaunation")) +
  scale_y_continuous(labels = scales::number_format(accuracy = 0.01), limits = c(1.5, NA), breaks = seq(1.5, 3, by = 0.25)) +
  theme(
    legend.title = element_blank(),
    legend.position = c(0.5, 0.2),
    legend.text = element_text(size = 16),
...
# Data frame with significant differences between interaction term treatment*time from summary(model)
#significant_months <- data.frame(
#  Time = c("48","54","60","66","72","78","84","96","108","156"),  # Actual significant months
#  Treatment = "Defaunation",
#  y_position = c(18, 18, 18, 18, 18, 18, 18, 18, 18, 18),  # Position for the annotations, adjust as needed
#  label = "*"
#)
...
ggplot(simpson_summary_data, aes(x = Time, y = mean_distance, color = Treatment, group = Treatment, fill = Treatment)) +
  ...
  scale_color_manual(values = custom_colors, labels = c("Control", "Defaunation")) +
  scale_fill_manual(values = custom_colors, labels = c("Control", "Defaunation")) +
  scale_y_continuous(labels = scales::number_format(accuracy = 0.01), limits = c(4, NA), breaks = seq(4, 20, by = 2)) +
  theme(
    legend.title = element_blank(),
    legend.position = c(0.5, 0.2),
    legend.text = element_text(size = 16),
...
##---------------------------------
## Função auxiliar: jackknife multi-site β
##---------------------------------
jackknife_multi_beta <- function(comm) {
  # comm: matriz comunidade (sites x espécies) já transformada (ex.: Hellinger)
  
  n_sites <- nrow(comm)
  if (n_sites < 3) stop("Need at least 3 sites for jackknife on multi-site beta.")
  
  # Beta multi-site observada com todos os sítios
...
# Labels T0, T6, ...
time_labels <- gsub("Itamambuca ", "", sheet_names)

# Data frame para guardar β multi-site + SE/IC jackknife
...
for (i in seq_along(sheet_list)) {
  ...
  for (tr in levels(treat_data$Treatment)) {
    ...
    if (length(idx) >= 3) {   # precisa de pelo menos 3 sítios por tratamento
