---
title: "Vargem Grande site analysis (VG)"
...
This R Markdown document reproduces the analyses for **VG** presented in the paper:
...
3. The script will save outputs to your **Desktop** (Windows) in a folder named `Defaunation_biotic_homogenization/VG`.
...
**Expected input file:** `data/Vargem_Grande_T0_T156.xlsx`
...
output_dir <- file.path(desktop_path, "Defaunation_biotic_homogenization", "VG")
...
excel_file <- "Vargem_Grande_T0_T156.xlsx"
# Data folder (relative to repository root)
data_dir <- "data"
excel_file_path <- file.path(data_dir, excel_file)

# Define the sheet names
sheet_names <- c("Vargem Grande T0", "Vargem Grande T6", "Vargem Grande T12", "Vargem Grande T18", "Vargem Grande T24", "Vargem Grande T30", "Vargem Grande T36", "Vargem Grande T42", "Vargem Grande T48", "Vargem Grande T54", "Vargem Grande T60", "Vargem Grande T66", "Vargem Grande T72", "Vargem Grande T78", "Vargem Grande T84", "Vargem Grande T96", "Vargem Grande T108", "Vargem Grande T156")
...
empty_df <- data.frame(matrix(NA, nrow = 24, ncol = 18))
...
for (i in seq_along(sheet_list)) {
  ...
  control <- spp_data[c(1:12),]
  treatment <- spp_data[c(13:24),]
  ...
  # Grouping
  diversity.results_richness[1:12,i+2]<-richness_control
  diversity.results_richness[13:24,i+2]<-richness_treatment
  
  diversity.results_shannon[1:12,i+2]<-shannon_index_control
  diversity.results_shannon[13:24,i+2]<-shannon_index_treatment
  
  diversity.results_simpson[1:12,i+2]<-simpson_index_control
  diversity.results_simpson[13:24,i+2]<-simpson_index_treatment
}
...
#  y_position = c(26),  # Position for the annotations, adjust as needed
#  label = "*"
#)
...
ggplot(simpson_summary_data, aes(x = Time, y = mean_distance, color = Treatment, group = Treatment, fill = Treatment)) +
  ...
  scale_color_manual(values = custom_colors, labels = c("Control", "Defaunation")) +
  scale_fill_manual(values = custom_colors, labels = c("Control", "Defaunation")) +
  scale_y_continuous(labels = scales::number_format(accuracy = 0.01), limits = c(8, NA), breaks = seq(8, 22, by = 2)) +
  theme(
    legend.title = element_blank(),
    legend.position = c(0.5, 0.2),
    legend.text = element_text(size = 16),
...
time_labels <- gsub("Vargem Grande ", "", sheet_names)
...
# (opcional) se ainda quiser guardar média par-a-par
